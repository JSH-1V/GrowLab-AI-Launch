    ...
    <>
      ...
      >
        <div className={`mx-auto flex max-w-7xl items-center justify-between rounded-full border border-white/10 bg-black/40 px-4 py-2 ${tier === "high" ? "backdrop-blur-xl" : "backdrop-blur-md"} md:px-6 md:py-3`}>
          <a href="#" className="flex items-center gap-2 font-accent text-sm tracking-tight transition-opacity hover:opacity-80">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-accent shadow-[0_0_15px_rgba(124,92,252,0.5)]">
              <Zap className="h-4 w-4 text-white" strokeWidth={2.5} />
            </div>
            <span className="mt-1">GrowLab AI</span>
          </a>

          {/* Desktop Links */}
          <div className="hidden items-center gap-8 text-[11px] font-accent uppercase tracking-wider text-white/60 md:flex">
            {navLinks.map((link) => (
              <a key={link.name} href={link.href} className="transition-all hover:text-white hover:tracking-widest">
                {link.name}
              </a>
            ))}
          </div>
          ...
          <div className="flex items-center gap-4">
            <MagneticButton
              ...
              className="hidden rounded-full bg-accent px-5 py-2 text-xs font-display font-bold text-white shadow-[0_0_20px_rgba(124,92,252,0.3)] transition-all hover:shadow-[0_0_30px_rgba(124,92,252,0.5)] sm:block"
            >
              Simular ecosistema
            </MagneticButton>
